"""Generator for gen/ids.ts — the TypeScript port of the frozen id mint.
...
_IDS_TS = '''// GENERATED from schema.json idScheme (schemaVersion "@VERSION@", schemaRevision "@REVISION@") by schemagen.py. Do not edit by hand.
...
//
// Hashing: sha256 over the UTF-8 preimage, hex, first @TRUNCATE@ chars.  On
...
export const US = @DELIMITER@;
export const US_ESCAPED = "\\\\x1f"; // how the delimiter is spelled in probe payloads
export const NODE_DOMAIN_TAG = "@NODE_TAG@";
export const EDGE_DOMAIN_TAG = "@EDGE_TAG@";
export const TRUNCATE = @TRUNCATE@;
export const NODE_ID_PREFIX = "@NODE_PREFIX@";
export const EDGE_ID_PREFIX = "@EDGE_PREFIX@";
...
def generate_ids_ts(schema_obj):
    f = _schema_facts(schema_obj)
    return _fill(_IDS_TS, {
        "VERSION": f["version"],
        "REVISION": f["revision"],
        "DELIMITER": json.dumps(f["delimiter"]),
        "TRUNCATE": str(f["hash_length"]),
        "NODE_TAG": f["node_tag"],
        "EDGE_TAG": f["edge_tag"],
        "NODE_PREFIX": f["node_id_prefix"],
        "EDGE_PREFIX": f["edge_id_prefix"],
    })
