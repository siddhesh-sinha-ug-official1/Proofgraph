"""Content-addressed IDs (Frozen Schema + the 7.7 hashing spec) — canonical mint.

Moved verbatim from graph-model src/ids.py into packages/schema (the ONE source
of truth for the assembly).  Semantics are frozen; `gen/ids.ts` is the byte-
agreeing TypeScript port and `vectors.json` is the cross-language golden gate.
